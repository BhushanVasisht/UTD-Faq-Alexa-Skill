...
class UtdCrawler(CrawlSpider):
    ...
    def parse_detail_page(self, response):
        question =  response.xpath('//div[@class="panel-heading"]/a/h4/text()').getall()
        ...
        for x in question:
            s = ""
            # a = s.join(response.xpath("//div[@class="panel-heading"][contains(., '%(x)s')]/following-sibling::node()/descendant-or-self::text()" % {'x': x}).extract())
            a = s.join(response.xpath("//div[@class='panel-heading'][contains(., '%(x)s')]/following-sibling::node()/descendant-or-self::text()" % {'x': x}).getall())
            a= a.strip()
            escapes = ''.join([chr(char) for char in range(1, 32)])
            a = a.translate(str.maketrans('','',escapes))
            answers.append(a)
        ...
        # print("Ques",question)
        print("Questions",question.__len__())
